---
name: internal-comms
description: Guidelines for writing effective internal communications (status reports, project updates, newsletters). Use when working with communication, reporting, status.
version: 1.0.0
category: utilities
triggers: [communication, reporting, status, faq, internal, comms]
dependencies: [doc-coauthoring]
inputs: [input document or dataset]
outputs: [generated file]
title: Internal Communications & Reporting
date: 2026-03-08
task_ref: initial-setup
confidence_score: 1.0
tags: [communication, reporting, status, faq, internal]
links: ["[[skills/communication/doc-coauthoring]]"]
---

# Internal Communications & Reporting

## 🎯 Purpose
Guidelines for writing effective internal communications (status reports, project updates, newsletters).

## 🛠️ The Process / Fact

### 1. Common Formats
- **3P Update:** Progress, Plans, Problems.
- **Company Newsletter:** Broad updates, announcements.
- **FAQ:** Answering frequently asked questions clearly.

### 2. Strategy
- **Audience First:** Match tone to the audience (leadership vs. teammates).
- **Structure:** Use the appropriate template (e.g., `examples/3p-updates.md`).

## ⚠️ Known Quirks or Edge Cases
- **Genericism:** Avoid vague updates. Be specific about blockers and "plans."

## 🔗 Related Memories
- [[skills/communication/doc-coauthoring]]
