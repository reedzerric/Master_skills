---
name: triage
description: 'Moves issues and external PRs through a state machine of triage roles: categorise, verify, grill when underspecified, and write agent-ready briefs. Use when the user wants their issue tracker inbox processed or an incoming PR assessed. Requires setup-engineering-flows to have configured the tracker.'
version: 1.0.0
category: core
triggers: [triage, process the inbox, categorise issues, incoming pr, issue backlog]
dependencies: [setup-engineering-flows]
inputs: [issues or PRs from the configured tracker]
outputs: [labelled issues, agent-ready briefs]
tags: [triage, issues, workflow]
...
These are canonical role names. The actual label strings used in the issue tracker may differ. The mapping should have been provided to you. If not, tell the user to run `/setup-engineering-flows`.
