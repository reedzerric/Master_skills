---
name: brand-guidelines
description: Guidelines for applying Anthropic's official brand colors and typography to artifacts. Use when working with media, design, branding.
version: 1.0.0
category: design_media
triggers: [media, design, branding, corporate, style, brand, guidelines]
dependencies: [theme-factory]
inputs: [brief, brand tokens]
outputs: [design artifact, style spec]
title: Anthropic Brand Guidelines
date: 2026-03-08
task_ref: initial-setup
confidence_score: 1.0
tags: [media, design, branding, corporate, style]
links: ["[[skills/media/theme-factory]]"]
---

# Anthropic Brand Guidelines

## 🎯 Purpose
Guidelines for applying Anthropic's official brand colors and typography to artifacts.

## 🛠️ The Process / Fact

### 1. Colors (Hex Codes)
- **Primary Text:** Dark (`#141413`)
- **Main Background:** Light (`#faf9f5`)
- **Secondary:** Mid Gray (`#b0aea5`)
- **Accents:** Orange (`#d97757`), Blue (`#6a9bcc`), Green (`#788c5d`).

### 2. Typography
- **Headings (24pt+):** Poppins (fallback: Arial).
- **Body Text:** Lora (fallback: Georgia).

## ⚠️ Known Quirks or Edge Cases
- **Legibility:** Adjust text color (Dark vs. Light) based on the background.
- **Cycle Accents:** Non-text shapes should cycle through the three accent colors (orange, blue, green).

## 🔗 Related Memories
- [[skills/media/theme-factory]]
